...
function MainPage({mood , onChangeMood, address,connectWallet,transactionStatus}:moodDiaryProps) {
  ...
  <p className='font-semibold text-[18px] sm:text-[20px] md:text-[24px] text-gray-900'>Mood Diary</p>
    <button className='px-2 py-2 text-white bg-gray-900 flex rounded-md' onClick={connectWallet}>{address ? address.slice(0,6).concat('...') :'Connect Wallet'}</button>
  </div>
  </div>
